...
//CRIA AS FOTOS DO INSTAGRAM
function getPhotoInstagram(media) {
  var photoContainer = $('.js-instagram ul')
  for(var i = 0; i < media.data.length; i++){
    photoContainer.append(
      '<li>' +
        '<a href="' + media.data[i].link + '" target="_blank">' +
          '<figure class="photo">' +
            '<img src="' + media.data[i].images.standard_resolution.url + '" width="250" />' +
            '<div class="stats">' + 
              '<span class="likes"><i class="fa fa-heart"></i>' + media.data[i].likes.count + '</span>' +
              '<span class="comments"><i class="fa fa-comment"></i>' + media.data[i].comments.count + '</span>' +
            '</div>' +
          '</figure>' +
        '</a>' +
      '</li>'
